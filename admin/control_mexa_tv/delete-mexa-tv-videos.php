...
if ($_SESSION['user']['level_id'] == 1 && $_GET['delete'] == 'video') {
  ...
  echo '
	<script>alert("Video is Successfully Deleted");</script>';
}
?>
    ...
    <div class="col-9 .bg-white">
      ...
      <div class='row'>
        <? foreach ($result as $row) { ?>
          <div class='col-3'>
            <a class='delete-video' data-id='<? echo $row['id']; ?>' href='#'><img alt='image' class='mt-3' src='https://img.youtube.com/vi/<? echo substr($row['url'], strpos($row['url'], 'embed') + 6, 11); ?>/0.jpg' alt='image'></a>
          </div>
        <? } ?>
      </div>
    </div>
...
<script>
  ...
  elements.forEach(ele => {
    ele.addEventListener('click', function(e) {
      ...
      if (isConfirmed) {
        window.location.search = `&delete=video&video_id=${id}`;
      }
    })
  })
</script>
