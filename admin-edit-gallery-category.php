...
$stmt = $conn->prepare("select title,id from gallery_categories");
$stmt->execute();
...
if (isset($_GET['event_id'])) {
  $stmt = $conn->prepare("select title,description from gallery_categories where id=?");
  ...
  <div class="row">
    ...
    <div class="col-9 .bg-white">
      <div class="breadcrumb-main">
        ...
        <p class="current-link">Edit Gallery Event</p>
      </div>
      <form class='mt-5 shadow-sm p-4 mb-5 bg-white rounded' method='post'>
        <div class='form-group'>
          <label for='category-title' class='font-weight-bold'>Select Gallery Event</label>
          <select id='category-title' name='category_id' class="custom-select" required>
            <option value="">Select Gallery Event</option>
...
<script>
    ...
    document.getElementById('category-title').addEventListener('change', function () {
        location.href = 'admin-edit-gallery-category.php?event_id=' + document.getElementById('category-title').value + '';
    })
</script>
