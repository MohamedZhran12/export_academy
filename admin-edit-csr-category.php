...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $stmt = $conn->prepare('update categories set title = ? , description= ? where id = ?');
...
if (isset($_GET['event_id'])) {
  $stmt = $conn->prepare("select title,description from categories where id=?");
    ...
    <div class="col-9 .bg-white">
      ...
      <form class='mt-5 shadow-sm p-4 mb-5 bg-white rounded' method='post'>
        <div class='form-group'>
          <label for='category-title' class='font-weight-bold'>Select CSR Event</label>
          <select id='category-title' name='category_id' class="custom-select" required>
            <option value="">Select CSR Event</option>
